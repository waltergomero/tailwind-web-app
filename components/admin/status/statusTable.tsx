...
const StatusTable = ({ data, onDelete }: StatusTableProps) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((status) => 
      status.status_name.toLowerCase().includes(lowerSearch) ||
      status.description?.toLowerCase().includes(lowerSearch) ||
      status.typeid.toString().includes(lowerSearch)
    );
  }, [data, searchTerm]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteStatus(selectedStatus.id);
                ...
                <TableRow key={status.id}>
                  ...
                  <TableCell className="px-4 py-2 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex items-center justify-center gap-4">
                        ...
                        <button 
                          onClick={() => handleDeleteClick(status.id, status.status_name)}
                          title="Delete status"
    ...
    <Modal isOpen={isDeleteModalOpen} onClose={handleDeleteCancel} className="max-w-md mx-4">
      <div className="p-6">
        ...
        <p className="text-sm text-center text-gray-600 dark:text-gray-400 mb-6">
          Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">{selectedStatus?.name}</span>? 
